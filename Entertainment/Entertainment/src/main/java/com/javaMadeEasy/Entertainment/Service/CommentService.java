...
public class CommentService {
    ...
    public String retrieveAllCommentsForAPost(@PathVariable Long postId){
        Optional<Post> postOptional= postRepository.findById(postId);
        if(!postOptional.isPresent()){
            throw  new RuntimeException("Post with id-"+postId+" was not found");
        }

        return postOptional.get().getBody();
    }
    
    
    public ResponseEntity<Comment> createComment(@PathVariable Long postId, @RequestBody Comment comment,int userId) {
